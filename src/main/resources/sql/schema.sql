...
CREATE TABLE user (id bigint NOT NULL, email VARCHAR(255), password VARCHAR(255), role VARCHAR(255), fk_cart bigint, PRIMARY KEY (id));
...
ALTER TABLE pedido ADD CONSTRAINT fk_userIdOnPedido FOREIGN KEY (fk_user) REFERENCES user(id);
ALTER TABLE user ADD CONSTRAINT fk_cartIdOnUser FOREIGN KEY (fk_cart) REFERENCES cart(id);
